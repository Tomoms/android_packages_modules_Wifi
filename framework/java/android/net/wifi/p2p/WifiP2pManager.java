...
public class WifiP2pManager {
    ...
    /**
     * Extra for transporting the WPS PIN or Wi-Fi Direct R2 pairing bootstrapping PIN/Password.
     * @hide
     */
    public static final String EXTRA_PARAM_KEY_PIN_OR_PASSWORD =
            "android.net.wifi.p2p.EXTRA_PARAM_KEY_PIN_OR_PASSWORD";

    /**
     * For credential type WPS PIN or Wi-Fi Direct R2 pairing bootstrapping PIN.
     * @hide
     */
    public static final int CREDENTIAL_TYPE_PIN = 1;

    /**
     * For credential type Wi-Fi Direct R2 pairing bootstrapping password.
     * @hide
     */
    ...
    /**
     * Extra for transporting the credential type, such as PIN or Password.
     * @hide
     */
    public static final String EXTRA_PARAM_KEY_CREDENTIAL_TYPE =
            "android.net.wifi.p2p.EXTRA_PARAM_KEY_CREDENTIAL_TYPE";
    ...
    /** @hide */
    public static final int EXTERNAL_APPROVER_PIN_OR_PASSWORD_GENERATED = BASE + 106;
   ...
   public interface DnsSdTxtRecordListener {
    ...
    public interface ExternalApproverRequestListener {
        ...
        /**
         * Called when a WPS PIN or Wi-Fi Direct R2 pairing bootstrapping PIN is generated by the
         * WiFi service.
         *
         ...
         *
         * @param deviceAddress is the peer MAC address used in the registration.
         * @param pin is a WPS PIN or a Wi-Fi Direct R2 pairing bootstrapping PIN.
         */
        void onPinGenerated(@NonNull MacAddress deviceAddress, @NonNull String pin);
        /**
         * Called when a Wi-Fi Direct R2 pairing bootstrapping password is generated by the WiFi
         * service.
         *
         * The external approver can display the password, exchange the password via Out-Of-Band way
         * or ask the wifi service to show the password as usual using the
         * {@link #setConnectionRequestResult(Channel, MacAddress, int, ActionListener)}
         * with {@link #CONNECTION_REQUEST_DEFER_SHOW_PASSWORD_TO_SERVICE}.
         *
         * @param deviceAddress is the peer MAC address used in the registration.
         * @param password is a Wi-Fi Direct R2 pairing bootstrapping password.
         */
        @FlaggedApi(Flags.FLAG_EXTERNAL_APPROVER_SUPPORT_FOR_WFDR2_PASSWORD_BASED_BOOTSTRAPPING)
        default void onPasswordGenerated(@NonNull MacAddress deviceAddress,
        ...
        class P2pHandler extends Handler {
            ...
            public void handleMessage(Message message) {
                ...
                switch (message.what) {
                    ...
                    case EXTERNAL_APPROVER_PIN_OR_PASSWORD_GENERATED:
                ...
                switch (message.what) {
                    ...
                    case EXTERNAL_APPROVER_PIN_OR_PASSWORD_GENERATED:
                        if (listener != null) {
                            ...
                            int credentialType =
                                    bundle.getInt(EXTRA_PARAM_KEY_CREDENTIAL_TYPE,
                                            CREDENTIAL_TYPE_PIN); // Default to PIN
                            String pinOrPassword = bundle.getString(
                                    EXTRA_PARAM_KEY_PIN_OR_PASSWORD);
                            if (Flags.externalApproverSupportForWfdr2PasswordBasedBootstrapping()
                                    && credentialType == CREDENTIAL_TYPE_PASSWORD) {
                                ((ExternalApproverRequestListener) listener)
                                        .onPasswordGenerated(deviceAddress, pinOrPassword);
                            } else { // Default or CREDENTIAL_TYPE_PIN
                                ((ExternalApproverRequestListener) listener)
                                        .onPinGenerated(deviceAddress, pinOrPassword);
                            }
                        }
                        break;
     ...
     */
    @FlaggedApi(Flags.FLAG_EXTERNAL_APPROVER_SUPPORT_FOR_WFDR2_PASSWORD_BASED_BOOTSTRAPPING)
    public static final int CONNECTION_REQUEST_DEFER_SHOW_PASSWORD_TO_SERVICE = 4;
    /** @hide */
     ...
     * @param deviceAddress the peer which is bound to the external approver.
     * @param result the response for the incoming request.
     * @param pinOrPassword the WPS PIN or Wi-Fi Direct R2 pairing bootstrapping PIN/Password
     *                      for the incoming request.
     * @param listener for callback on success or failure.
     */
    @RequiresPermission(android.Manifest.permission.MANAGE_WIFI_NETWORK_SELECTION)
    public void setConnectionRequestResult(@NonNull Channel c, @NonNull MacAddress deviceAddress,
            @ConnectionRequestResponse int result, @Nullable String pinOrPassword,
            @Nullable ActionListener listener) {
        checkChannel(c);
        if (null == deviceAddress) {
            throw new IllegalArgumentException("deviceAddress cannot be empty");
        }
        if (result == CONNECTION_REQUEST_ACCEPT && TextUtils.isEmpty(pinOrPassword)) {
            throw new IllegalArgumentException("PIN cannot be empty for accepting a request");
        }

        Bundle extras = prepareExtrasBundle(c);
        extras.putParcelable(EXTRA_PARAM_KEY_PEER_ADDRESS, deviceAddress);
        extras.putString(EXTRA_PARAM_KEY_PIN_OR_PASSWORD, pinOrPassword);
        c.mAsyncChannel.sendMessage(prepareMessage(SET_CONNECTION_REQUEST_RESULT,
                result, c.putListener(listener), extras, c.mContext));
    }
